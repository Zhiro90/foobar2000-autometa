// ============================================================================
// SCRIPT NAME: Quick Tag Grouper (Final Release - RGB Text Fix)
// ============================================================================
...
// Helper para convertir texto "255,255,255" a Color real
...
var theme_mode = window.GetProperty("Theme Mode", 0); // 0=Dark, 1=System, 2=Custom
...
// --- ESTADO ---
var hover_zone = 0; // 0=None, 1=Bolt, 2=Text, 3=Menu
var btn_width = 30;
...
var display_mode = window.GetProperty("Display Mode", 1); // 0=Minimal, 1=Info
var auto_play = window.GetProperty("Auto Play", false); 
var viewMode = window.GetProperty("View Mode", 0); // 0=Folders, 1=Flat
...
function on_paint(gr) {
    ...
    if (display_mode === 0) {
        ...
        // Hover central
        if (hover_zone === 2) {
             gr.FillSolidRect(btn_width, 0, w - (btn_width * 2), h, 0x15ffffff); 
        }

        if (handle) {
            var titleStr = tf_title.EvalWithMetadb(handle);
            var artistStr = tf_artist.EvalWithMetadb(handle);
            gr.GdiDrawText(titleStr, font_title, c_txt, txt_x, 0, txt_w, h/2 + 2, 1 | 8 | 32768); 
            gr.GdiDrawText(artistStr, font_artist, c_dim, txt_x, h/2, txt_w, h/2 - 2, 1 | 4 | 32768);
        } else {
...
function on_mouse_move(x, y) {
    ...
    if (display_mode === 0) {
        if (x > w - btn_width) hover_zone = 3;
        else hover_zone = 1;
    } else {
        if (x < btn_width) hover_zone = 1;
        else if (x > w - btn_width) hover_zone = 3;
        else hover_zone = 2;
    }
...
function run_quick_action() {
    var handle = fb.GetNowPlaying();
    if (!handle) return;
    var opt = { field: qa_field, type: qa_type, rawPattern: qa_pattern, value: "" };
    
    if (opt.type === "meta") {
        var idx = handle.GetFileInfo().MetaFind(opt.field);
        if (idx !== -1) opt.value = handle.GetFileInfo().MetaValue(idx, 0);
    ...
    if (opt.value) create_playlist(opt);
}

function create_playlist(opt) {
...
function show_context_menu(x, y) {
    ...
    for (var c = 0; c < commonTags.length; c++) {
        ...
        if (idxTag !== -1) addItem(mainMenu, commonTags[c], fileInfo.MetaValue(idxTag, 0), "meta", "");
    }
    mainMenu.AppendMenuItem(0, 0, "---------------------------");
    ...
    for (var i = 0; i < fileInfo.MetaCount; i++) {
        addItem(targetMeta, fileInfo.MetaName(i).toUpperCase(), fileInfo.MetaValue(i, 0), "meta", "");
    }
    ...
    var sysTags = [
        {n: "FILE SIZE", p: "%filesize_natural%"},
        {n: "EXACT SIZE", p: "%filesize%"},
        {n: "DURATION", p: "%length%"},
        {n: "LAST MODIFIED", p: "%last_modified%"},
        {n: "PATH", p: "%path%"},
        {n: "FILENAME", p: "%filename_ext%"},
        {n: "FOLDER NAME", p: "$directory_path(%path%)"},
        {n: "REPLAYGAIN TRACK", p: "%replaygain_track_gain%"},
        {n: "REPLAYGAIN ALBUM", p: "%replaygain_album_gain%"},
        {n: "EXTENSION", p: "%filename_ext%"}
    ];

    for (var k=0; k<sysTags.length; k++) {
        var val = fb.TitleFormat(sysTags[k].p).EvalWithMetadb(handle);
        if (val && val !== "?" && val !== "") {
            addItem(targetStat, sysTags[k].n, val, "system", sysTags[k].p);
        }
    }
    ...
    if (validOptions[choice]) {
        ...
        if (utils.IsKeyPressed(0x10)) { // Shift
